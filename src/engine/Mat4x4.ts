...
export class Mat4x4{
  m:Mat4x4.M = Array.from({ length: 4}, () => Array(4).fill(0)) as Mat4x4.M;


  multiplyBy({ m:mm }:Mat4x4){
    const { m } = this;
    const matrix = new Mat4x4();

    for (let c = 0 as Mat4x4.Index; c < 4; c++){
      for (let r = 0 as Mat4x4.Index; r < 4; r++){
        matrix.m[r][c] = (
          (m[r][0] * mm[0][c]) +
          (m[r][1] * mm[1][c]) +
          (m[r][2] * mm[2][c]) +
  ...
  quickInverse():Mat4x4{
    ...
    matrix.m[0][0] = m[0][0]; matrix.m[0][1] = m[1][0]; matrix.m[0][2] = m[2][0]; matrix.m[0][3] = 0;
		matrix.m[1][0] = m[0][1]; matrix.m[1][1] = m[1][1]; matrix.m[1][2] = m[2][1]; matrix.m[1][3] = 0;
		matrix.m[2][0] = m[0][2]; matrix.m[2][1] = m[1][2]; matrix.m[2][2] = m[2][2]; matrix.m[2][3] = 0;
  ...
  static rotateX(angleRad:number):Mat4x4{
    const matrix = new Mat4x4();
  ...
  static rotateY(angleRad:number):Mat4x4{
    const matrix = new Mat4x4();
  ...
  static rotateZ(angleRad:number):Mat4x4{
    const matrix = new Mat4x4();
  ...
  static rotate({x,y,z}:Vec3D.Kurz, type:("deg"|"rad") = "rad"):Mat4x4{
  ...
  static translate({x,y,z}:Vec3D.Kurz){
    const matrix = Mat4x4.identity();
  ...
  static projection(fovDegrees:number, aspectRatio:number, near:number, far:number):Mat4x4{
    const matrix = new Mat4x4();
  ...
  static pointAt(pos:Vec3D,target:Vec3D,up:Vec3D):Mat4x4{
 ...
 */
export function toRadian(a:number) { return a * degree; }
/** ## toDegree
 * ---
 * 
 * Convert radians to degrees
 */
export function toDegree(a:number) { return a * radian; }