...
export class CanvasGameEngine {
    ...
    print(x, y, colour, message) {
        const { context } = this;
        context.font = "24px serif";
        context.fillStyle = colour;
        context.fillText(message, x, y);
    }
    drawTriangle(x0, y0, x1, y1, x2, y2, colour, width) {
        const { context } = this;
    ...
    drawLookLine2D(x0, y0, x2, y1, colour, lineWidth) {
        const { context } = this;
        context.beginPath();
        context.moveTo(x0, y0);
    ...
    onUserUpdate(fElapsedTime) {
        ...
        for (const triangle of trianglesToRaster) {
            ...
            this.fillTriangle(p[0].x, p[0].y, p[1].x, p[1].y, p[2].x, p[2].y, colour);
...
function interpolateColorFn(startColour, endColour) {
